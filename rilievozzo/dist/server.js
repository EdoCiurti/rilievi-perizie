...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        if (!user)
            return res.status(401).send({ message: 'Utente non trovato' });
...
app.post('/api/cambiaPassword', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // Verify old password
        const match = user.password.startsWith('$2')
            ? await bcrypt_1.default.compare(vecchiaPassword, user.password)
        ...
        const saltRounds = 10;
        const hashedPassword = await bcrypt_1.default.hash(nuovaPassword, saltRounds);
        // Update the password
...
app.get('/api/perizie', authenticateToken, async (req, res) => {
    if (!connectionString) {
        return res.status(500).send({ message: 'Database connection string non definita' });
    }
    const client = new mongodb_1.MongoClient(connectionString);
...
app.get('/api/perizie/:id', authenticateToken, async (req, res) => {
    if (!connectionString) {
        return res.status(500).send({ message: 'Database connection string non definita' });
    }
    const { id } = req.params;
    ...
    try {
        await client.connect();
        const collection = client.db(DB_NAME).collection('perizie');
        const perizia = await collection.findOne({ _id: new mongodb_1.ObjectId(id) });
        if (!perizia) {
...
app.post('/api/perizie', authenticateToken, upload.array('immagini', 10), async (req, res) => {
    try {
        ...
        if (req.files && req.files.length > 0) {
            ...
            req.files.forEach((file, index) => {
                immagini.push({
                    _id: new mongodb_1.ObjectId(), // <-- aggiungi questo
...
app.put('/api/perizie/:id', authenticateToken, upload.array('immagini'), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (operatore)
            updateDoc.operatore = operatore;
        if (coordinate)
            updateDoc.coordinate = coordinate;
        ...
        if (req.body.immaginiDaEliminare) {
            ...
            for (const imgData of immaginiDaEliminareArray) {
                ...
                const immagineIndex = perizia.immagini.findIndex(img => img._id.toString() === imgId.toString());
                if (immagineIndex !== -1) {
                    ...
                    if (imgFilename) {
                        const filePath = path_1.default.join('./uploads', imgFilename);
        ...
        await collection.updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: updateDoc });
        client.close();
        res.status(200).json({
            success: true,
...
app.delete('/api/perizie/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (req.user.role === 'admin') {
            ...
            if (admin.password.startsWith('$2')) {
                ...
                isPasswordValid = await bcrypt_1.default.compare(adminPassword, admin.password);
            }
            else {
            ...
            const result = await perizieCollection.deleteOne({ _id: new mongodb_1.ObjectId(id) });
            client.close();
...
app.put('/api/clienti/aggiorna', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await collection.updateMany({ cliente: nomeOriginale }, { $set: { cliente: nuovoNome } });
        client.close();
...
app.get('/api/users/profile', authenticateToken, async (req, res) => {
    try {
        ...
        const user = await collection.findOne({ _id: new mongodb_1.ObjectId(userId) }, { projection: { password: 0 } });
        if (!user) {
...
app.put('/api/users/profile', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await collection.updateOne({ _id: new mongodb_1.ObjectId(userId) }, { $set: updateFields });
...
app.put('/api/perizie/modifica', authenticateToken, upload.array('immagini'), async (req, res) => {
    try {
        ...
        try {
            ...
            if (immaginiDaEliminare) {
                ...
                if (perizia.immagini && Array.isArray(perizia.immagini)) {
                    updateDoc.immagini = perizia.immagini.filter((img) => {
                        const deveEliminare = immaginiDaEliminareArray.includes(img._id.toString());
                        if (deveEliminare) {
                            // Estrai il nome del file dall'URL
                            const filename = img.url.split('/').pop();
                            if (filename) {
                                fs_1.default.unlink(path_1.default.join('./uploads', filename), (err) => {
                                    if (err)
